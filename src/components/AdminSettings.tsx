...
const AdminSettings: React.FC<AdminSettingsProps> = ({ onClose, onUpdate }) => {
  ...
  const moveCategory = (idx: number, direction: "up" | "down") => {
    if (
      (direction === "up" && idx === 0) ||
      (direction === "down" && idx === config.categories.length - 1)
    )
      return;
    const newCats = [...config.categories];
  ...
  const addOrUpdateSortOption = () => {
    ...
    if (newSortLabel.trim()) {
      if (editingSortId) {
        ...
        setConfig({
          ...config,
          sortOptions: [...config.sortOptions, newOption],
        });
      }
  ...
  const deleteSortOption = (id: string) => {
    setConfig({
      ...config,
      sortOptions: config.sortOptions.filter((opt) => opt.id !== id),
    });
  ...
  const toggleUserFieldActive = (id: string) => {
    ...
    if (field && (field.key === "username" || field.key === "password")) {
      setError(
        "El usuario y la contraseña son obligatorios y no se pueden ocultar.",
      );
      return;
    }
      ...
      <div className="bg-neutral-800 border-2 border-wood w-full max-w-2xl rounded-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-neutral-700 flex justify-between items-center bg-neutral-900 rounded-t-lg">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <Settings className="text-wood animate-spin-slow" /> Configuración
            del Sistema
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-grow overflow-y-auto p-6 bg-neutral-800 custom-scrollbar">
          ...
          {activeTab === "categories" && (
            <div className="space-y-4">
              <p className="text-sm text-neutral-400 mb-2">
                Gestiona las categorías de incidencias. Reordénalas para cambiar
                el listado.
              </p>
              ...
              <div className="space-y-2">
                {config.categories.map((cat, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 bg-neutral-900 p-2 rounded border border-neutral-700 group"
                  >
                    <div className="flex flex-col gap-1 text-neutral-500">
                      <button
                        onClick={() => moveCategory(idx, "up")}
                        disabled={idx === 0}
                        className="hover:text-wood disabled:opacity-30"
                      >
                        <ArrowUp size={14} />
                      </button>
                      <button
                    ...
                    <div className="flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => setEditingCategory({ idx, val: cat })}
          ...
          {activeTab === "sorting" && (
            <div className="space-y-4">
              <p className="text-sm text-neutral-400 mb-2">
                Crea, edita o activa reglas de ordenación.
              </p>

              {/* Create/Edit Sort Box */}
              <div
                className={`p-3 rounded border mb-4 ${editingSortId ? "bg-wood/20 border-wood" : "bg-neutral-900 border-neutral-700"}`}
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
                  ...
                  <div className="flex gap-2">
                    <select
                      value={newSortField}
                      onChange={(e) =>
                        setNewSortField(e.target.value as keyof Incident)
                      }
                ...
                <div className="flex gap-2">
                  ...
                  {editingSortId && (
                    <button
                      onClick={cancelEditSort}
                      className="px-3 bg-neutral-700 hover:bg-neutral-600 text-white rounded font-bold text-sm"
                    >
                      Cancelar
                    </button>
                  )}
              ...
              <div className="bg-neutral-900 rounded-lg border border-neutral-700 overflow-hidden">
                {config.sortOptions.length === 0 && (
                  <p className="p-4 text-center text-neutral-500 italic">
                    No hay opciones de ordenación.
                  </p>
                )}
                {config.sortOptions.map((opt) => (
                  <div
                    key={opt.id}
                    className="flex items-center justify-between p-3 border-b border-neutral-700 last:border-0 hover:bg-neutral-800 transition-colors group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span
                          className={`font-bold block ${opt.active ? "text-neutral-200" : "text-neutral-500"}`}
                        >
                          {opt.label}
                        </span>
                        <span className="text-xs text-neutral-500 font-mono">
                          {FIELD_LABELS.find((f) => f.key === opt.field)
                            ?.label || opt.field}{" "}
                          ({opt.direction === "asc" ? "A-Z" : "Z-A"})
                        </span>
                      </div>
                    </div>
                    <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => startEditSort(opt)}
                        className="text-blue-400 hover:text-blue-300 p-2"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
                        onClick={() => deleteSortOption(opt.id)}
          ...
          {activeTab === "users" && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-neutral-400 mb-2">
                Personaliza el formulario de registro de vecinos. Oculta, edita
                etiqueta y placeholder, o añade campos nuevos.
              </p>
              ...
              <div className="bg-neutral-900 p-3 rounded border border-neutral-700 mb-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewUserFieldPlaceholder(e.target.value)
                      }
                      className="w-full bg-neutral-800 border border-neutral-600 rounded p-2 pl-8 text-white placeholder-neutral-500 focus:border-wood outline-none text-sm"
                ...
                <button
                  ...
                  className="w-full bg-wood text-white py-1.5 rounded font-bold text-sm hover:bg-wood-hover flex items-center justify-center gap-2"
                >
              ...
              <div className="space-y-2">
                {config.userFields.map((field) => (
                  <div
                    key={field.id}
                    className="flex items-center justify-between bg-neutral-900 p-3 rounded border border-neutral-700 group transition-all hover:border-neutral-600"
                  >
                    ...
                    <div className="flex items-center gap-3 flex-grow w-full">
                      <button
                        ...
                        title={
                          field.active ? "Visible en formulario" : "Oculto"
                        }
                      >
                        {field.key === "username" ||
                        field.key === "password" ? (
                      ...
                      {editingUserFieldId === field.id ? (
                        <div className="flex-grow grid grid-cols-1 sm:grid-cols-2 gap-2 mr-2">
                          <div className="relative">
                            <Type
                              size={14}
                              className="absolute left-2 top-2 text-wood"
                            />
                            <input
                        ...
                        <div className="flex flex-col">
                          <span
                            className={`font-bold text-sm flex items-center gap-2 ${field.active ? "text-neutral-200" : "text-neutral-500 line-through"}`}
                          >
                            {field.label}
                            {field.isSystem && (
                              <span className="text-[9px] bg-neutral-800 border border-neutral-600 px-1 rounded text-neutral-400 uppercase">
                                Sistema
                              </span>
                            )}
                          </span>
                      ...
                      {editingUserFieldId === field.id ? (
                        <>
                          <button
                            ...
                            className="text-green-500 p-1 bg-neutral-800 rounded hover:bg-neutral-700 border border-transparent hover:border-green-900"
                          >
                            <Check size={18} />
                          </button>
                          <button
                            onClick={() => setEditingUserFieldId(null)}
                            className="text-red-500 p-1 bg-neutral-800 rounded hover:bg-neutral-700 border border-transparent hover:border-red-900"
                          >
                            <X size={18} />
                          </button>
                        </>
                        ...
                        <div className="flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            title="Editar Etiqueta y Placeholder"
                          >
                            <Edit2 size={16} />
                          </button>
                          {!field.isSystem && (
