...
import HouseRegistry from "./HouseRegistry"; // <--- CAMBIO 1: Importar el componente
import {
  RefreshCcw,
  Filter,
  Mail,
  ...
  HelpCircle,
  Home,
} from "lucide-react"; // <--- CAMBIO 1: Añadido 'Home'
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [showHouseRegistry, setShowHouseRegistry] = useState(false); // <--- CAMBIO 2: Estado para el censo
  ...
  const fetchData = async () => {
    ...
    if (user.status === "pending") {
      if (appConfig)
        setPendingMessage(
          appConfig.pendingAccountMessage || "Cuenta pendiente.",
        );
      setLoading(false);
  ...
  const handleExport = (format: "pdf" | "excel") => {
    ...
    const currentSortLabel =
      availableSortOptions.find((o) => o.id === sortOptionId)?.label ||
      "Ordenación actual";
    alert(
      `Descargando listado en ${formatName} con ${filteredIncidents.length} incidencias. (${currentSortLabel})... (Simulación)`,
    );
  };
    ...
    <div className="relative">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 gap-4">
            <div className="flex items-center gap-4">
              <h2 className="text-xl font-bold text-wood flex items-center gap-2">
                <UserPlus size={24} /> Solicitudes de Acceso (
                {pendingUsers.length})
              </h2>
            ...
            {selectedPendingIds.length > 0 ? (
              ...
              <span className="text-xs font-bold text-neutral-500 uppercase tracking-widest hidden md:block">
                Revisión Requerida
              </span>
            )}
          </div>
                ...
                >
                  ...
                  <div className="mb-4 pr-8">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-3 bg-neutral-900/50 p-3 rounded pointer-events-none">
                      ...
                      {pu.custom_fields &&
                        Object.entries(pu.custom_fields).map(([k, v]) => (
                          <div
                            key={k}
                            className="col-span-2 border-t border-neutral-700 pt-1 mt-1"
                          >
                            <span className="text-[10px] text-neutral-500 uppercase block">
                              {k}
                            </span>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2 bg-neutral-200 px-3 py-1.5 rounded-lg border border-neutral-400 shadow-sm">
            ...
            >
              ...
              {categories.map((cat) => (
                <option key={cat} value={cat} className="text-neutral-900">
                  {cat}
                </option>
              ))}
            </select>
          ...
          {canManageUsers && (
            <div className="flex gap-2">
              ...
              {/* <--- CAMBIO 3: Botón del Censo (Casas) --- */}
              <button
                onClick={() => setShowHouseRegistry(true)}
                className="p-2 rounded border border-neutral-600 bg-neutral-800 text-neutral-400 hover:text-white hover:border-wood transition-colors"
                title="Ver Censo de Casas (1-45)"
              >
                <Home size={18} />
              </button>
              {/* ------------------------------------------ */}
      ...
      {showViewConfig && canManageUsers && (
        <div className="mb-4 bg-neutral-800 p-4 rounded border border-wood/50 animate-in slide-in-from-top-2">
          <h3 className="text-wood font-bold text-sm mb-3 flex items-center gap-2">
            <Eye size={16} /> Campos Visibles en el Listado
          </h3>
          <div className="flex flex-wrap gap-4">
            {Object.keys(viewConfig).map((key) => (
              <label
                key={key}
                className="flex items-center gap-2 cursor-pointer text-sm text-neutral-300 select-none"
              >
                <input
                  type="checkbox"
                  ...
                  onChange={() =>
                    setViewConfig((prev) => ({
                      ...prev,
                      [key]: !prev[key as keyof IncidentViewConfig],
                    }))
                  }
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-neutral-800 border-2 border-neutral-700 w-full max-w-lg rounded-lg shadow-2xl p-6 relative card">
            ...
            <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
              <Mail className="text-wood" /> Enviar Informe a Vecinos
            </h2>
            <textarea
              value={emailMessage}
          ...
          <div className="bg-[#FAF7F2] dark:bg-[#303134] border-2 border-wood dark:border-neutral-600 w-full max-w-lg rounded-lg shadow-2xl p-6 relative card">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-4 mb-6 text-sm text-blue-900 dark:text-blue-200">
              <p className="font-bold flex items-center gap-2 mb-2">
                <HelpCircle size={16} /> Formato Requerido:
              </p>
              ...
              <p className="text-xs mt-2 italic">
                Nota: Para la imagen, proporciona una URL pública (https://...)
                o déjalo vacío. No se pueden leer rutas locales (C:\...) por
            ...
            <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-neutral-300 dark:border-neutral-600 rounded-lg bg-white/50 dark:bg-black/20 hover:bg-white dark:hover:bg-black/30 transition-colors cursor-pointer group">
              {importing ? (
                ...
                <label className="flex flex-col items-center w-full h-full cursor-pointer">
                  <Upload
                    size={32}
                    className="text-neutral-400 group-hover:text-wood mb-2 transition-colors"
                  />
                  <span className="font-bold text-neutral-600 dark:text-neutral-300 text-sm">
                    Haz clic para seleccionar tu CSV
                  </span>
                  <input
                    type="file"
                    accept=".csv"
          ...
          <div className="bg-neutral-800 border-2 border-neutral-700 w-full max-w-sm rounded-lg shadow-2xl p-6 relative card">
            ...
            <div className="mt-4 text-center">
              <p className="text-xs text-neutral-500">
                Se exportarán {filteredIncidents.length} incidencias.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* <--- CAMBIO 4: Renderizar el Modal del Censo --- */}
      {showHouseRegistry && (
